...
# ## Data scriping
# content = "Trump" # content to search
...
def keywordScan(lim):
	'''
	Function to scrape the first lim keywords. lim is the number of keywords required.
	'''
	n = 0 # starts from the first item
	kw_list = [] # a list to save the keywords
	while n < lim: # loop until reaching the required number of keywords
...
while keep_going == True:
	...
	if len(data["response"]["docs"]) == 0: # when there is no docs
		...
		for doc_num in range(doc_total):
			...
			with open("./trump.csv", "a") as my_csv: # “a” means append
